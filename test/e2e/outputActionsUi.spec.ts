...
test('actions-v1 still renders the legacy Output Actions auxiliary channel (choices, controls)', async () => {
  ...
  app = await launchAppWithWorkflow({
    ...
    nodes: [
      ...
      node('choices', 'write-text', 350, {
        writeTextValue: JSON.stringify({ choices: [{ type: 'buttons', prompt: 'What do you do?', options: ['Fight', 'Flee'] }] }),
      }),
    ...
    edges: [
      ...
      { id: 'aux', source: 'choices', target: 'output', targetHandle: 'output-actions' },
    ],
  }, {
  ...
  await electronApp.evaluate(({ ipcMain }) => {
    ...
    ipcMain.handle('llm:chat-completion', (_event, request) => {
      ...
      return { text: JSON.stringify({ version: 1, catalogId: catalog.catalogId, blocks: [
        { type: 'text', text: 'Alice glances at the door, weighing her options.' },
  ...
  await page.getByPlaceholder('Click here or press Enter to write. Type /cmd for commands').fill('Alice pauses at the doorway.');
  await page.getByRole('button', { name: /Run Chat/ }).click();
  await expect(page.getByText('Alice glances at the door, weighing her options.', { exact: true })).toBeVisible({ timeout: 20000 });
  ...
  await expect(page.getByRole('button', { name: 'Flee', exact: true })).toBeVisible();
});
