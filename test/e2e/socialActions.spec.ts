...
function storyWithFotogramAccount() {
...
test('actions-v1 commits one real Fotogram post and one comment on it', async () => {
  const story = storyWithFotogramAccount();
  ...
  await electronApp.evaluate(({ ipcMain }) => {
    ...
    ipcMain.handle('llm:chat-completion', (_event, request) => {
      ...
      if (!alice?.capabilities?.includes('fotogram.social')) throw new Error('Fotogram capability missing from the action catalog.');
      return { text: JSON.stringify({ version: 1, catalogId: catalog.catalogId, blocks: [
        { type: 'text', text: 'Alice opens Fotogram.' },
        { type: 'action', intent: { type: 'social.post', app: 'fotogram', author: alice.handle, caption: 'Sunset at the pier.' } },
        { type: 'action', intent: { type: 'social.comment', app: 'fotogram', author: alice.handle, postId: 'fotogram-post-01', text: 'So pretty!' } },
  ...
  await page.getByPlaceholder('Click here or press Enter to write. Type /cmd for commands').fill('Alice posts a photo.');
  await page.getByRole('button', { name: /Run Chat/ }).click();
  await expect(page.getByText('Alice opens Fotogram.', { exact: true })).toBeVisible({ timeout: 20000 });
  const postCard = page.getByRole('button', { name: 'Open Fotogram post by Alice with comments' });
  await expect(postCard).toBeVisible();
  await expect(postCard).toContainText('Sunset at the pier.');
  // One real comment was appended to the post's reaction thread.
  await postCard.click();
  await expect(page.getByText('So pretty!', { exact: true })).toBeVisible();
  expect(errors).toEqual([]);
  await expect.poll(async () => {
    const result = await page.evaluate(() => window.rpgraph.loadTurnAutosave());
    return JSON.stringify(result);
  }).toContain('Sunset at the pier.');
  ...
  expect(session.timeline.filter((entry) => entry.text?.original.includes('Sunset at the pier.'))).toHaveLength(1);
  expect(session.timeline.filter((entry) => entry.text?.original.includes('commented on fotogram-post-01'))).toHaveLength(1);
  expect(await page.evaluate(() => window.rpgraph.readEffectJournal())).toEqual([]);
});

test('staged workflow v1 commits one real Fotogram post as a receipt beat', async () => {
  const story = storyWithFotogramAccount();
  const node = (id: string, type: keyof typeof currentCoreNodeVersions, x: number, data = {}) => ({
    id, type: 'workflow', position: { x, y: 40 },
    data: { nodeType: type, nodeDataVersion: currentCoreNodeVersions[type], label: id, description: '', preview: '', connectionId: 'test', ...data },
    ...
    ipcMain.handle('llm:chat-completion', (_event, request: { prompt: string }) => {
      if (request.prompt.includes('Return one complete JSON compact plan')) {
        ...
        const plan = {
          ...
          instances: [{
            id: 'post', recipe: 'social.post', purpose: 'Post a photo.', actorId: alice, visibility, args: { app: 'fotogram' },
          ...
          beats: [{ id: 'b-post', visibility, content: [{ source: 'output', instanceId: 'post', output: 'receipt', kind: 'receipt' }],
            requiresReceipts: [{ source: 'output', instanceId: 'post', output: 'receipt', kind: 'receipt' }] }],
        };
      ...
      if (request.prompt.includes('Stage: social.post/draft')) {
        return { text: 'Sunset at the pier.', stats: { elapsedMs: 1, promptTokens: 20, completionTokens: 10 } };
      }
