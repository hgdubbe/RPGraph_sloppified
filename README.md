...
`main` now carries the UI overhaul plus the kept backend features listed below (LLM providers, turn autosave, NPC
auto-unlock). The old action-runtime/staged/decision-workflow direction was abandoned and dropped before this merge, 
not carried forward because the Maintainer of the original uploaded a much better solution. 
They remain in the merge-v0.5.0-decision-router Branch which is stale.
