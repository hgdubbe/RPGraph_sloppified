...
export function rpStorybookEditPrompt(currentJson: string, instruction: string, identityLocked = false) {
  return [
    ...
    'If the user asks for edits or provides new story facts, edit only the required fields. Preserve unrelated values, including characters[].decisionSettings. Image galleries, character profileImage portraits, voice samples, and phoneSettings are managed by app controls: never patch them, even on request; explain which app controls to use instead. Image-generation text in comfyConfig can be edited on request.',
