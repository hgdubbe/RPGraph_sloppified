...
export function applyJsonPatchOperation(target: unknown, operation: JsonPatchOperation) {
  ...
  const canonicalPath = (path: unknown) => typeof path === 'string' ? path.replace(
    /(\/apps\/(?:fotogram|onlyfriends|matchme))\/(?:username|displayName)$/, '$1/profileName') : path;
  operation = { ...operation, path: canonicalPath(operation.path)!,
    ...('from' in operation ? { from: canonicalPath(operation.from) } : {}) };
...
export function parseRpStorybookAssistantResult(text: string, fallback: RpStorybook): RpStorybookAssistantResult {
  ...
  const parsed = recordValue(parseStorybookAssistantJson(stripped.slice(start, end + 1)));
