...
function character(id: string, name: string, social: Partial<StorybookCharacter['social']> = {}): StorybookCharacter {
  return {
    id, name, storybookNodeId: 'story', kind: 'character', sourceId: id, label: name,
    ...
    social: { ...defaultRpStorybookCharacterSocial(), ...social },
  };
}

describe('legacy social output commits', () => {
  const characters: StorybookCharacter[] = [character('mia', 'Mia')];
  ...
  it('builds a comment without mutating the post or invoking effects', () => {
    ...
    const outsidePerson = character('outside', 'Outside Person', { fotogramUsername: 'outside.person' });
    const result = buildSocialCommentCommit({ incoming: { app: 'fotogram', postId: 'post-1', from: 'Outside Person', text: 'Nice' }, characters: [outsidePerson], messages });
  ...
  it('rejects missing recipients and self messages', () => {
    const alice = character('alice', 'Alice', { fotogramUsername: 'alice' });
    const base = { characters: [alice], messages: [], messageId: 'dm-1', sentAt: '2026-09-07T00:00:00Z' };
  ...
  it('retains explicit IDs, default recipient, tips, post origin and legacy missing-post warning', () => {
    const alice = character('alice', 'Alice', { onlyfriendsUsername: 'alice' });
    const bob = character('bob', 'Bob', { onlyfriendsUsername: 'bob' });
    const base = { characters: [alice, bob], messages: [], messageId: 'dm-2', sentAt: '2026-09-07T00:00:00Z', defaultRecipient: { name: 'Bob', handle: 'bob' } };
