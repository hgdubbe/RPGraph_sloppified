...
describe('social identity resolution', () => {
  ...
  it('tolerates a punctuation slip in an existing character\'s handle and canonicalizes it in history', () => {
    const owner = character('espen', 'Espen Harper', { fotogramUsername: '', onlyfriendsUsername: 'espen.harper' });
    const rico = character('rico', 'Rico Fisher', { fotogramUsername: '', onlyfriendsUsername: 'rico.fisher' });
    const message: SocialDirectMessageRecord = { app: 'onlyfriends', messageId: 'first',
      from: rico.name, fromHandle: 'ricofisher', to: owner.name, toHandle: 'espen.harper',
      text: 'Hey!', sentAt: '2026-09-16T01:00:00Z' };
    const canonical = canonicalSocialDirectMessage(message, [owner, rico], []);
    expect(canonical.fromHandle).toBe('rico.fisher');
