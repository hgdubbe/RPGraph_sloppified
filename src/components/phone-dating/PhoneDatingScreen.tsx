    ...
    <main ref={mainRef} className={`pt-main${selectedMatch && !editing ? ' pt-chat-main' : !editing && tab === 'discover' ? ` pt-discover-main${isScrollMode ? ' pt-scroll-mode' : ''}` : ''}`}>
      {!owner ? <div className="pt-empty"><h2>Who’s holding the phone?</h2><p>Select a Storybook character to create a profile.</p></div> : editing ?
        <form className="pt-form pt-form-compact" onSubmit={(event) => {
          ...
          <div className="pt-field-row"><label className="pt-name-label">Display name<input readOnly className="pt-readonly-name" value={owner.name} title="Name is locked to character" tabIndex={-1} /></label>
          ...
          <div><div className="pt-photo-heading"><strong>Manage photos</strong><small>{draft.photoIds.length}/{datingPhotoLimit} · At least one required</small></div>
            ...
            <div className="pt-photo-actions"><button type="button" disabled={profileOnly || busy || draft.photoIds.length >= datingPhotoLimit} onClick={() => uploadRef.current?.click()}>{busy ? 'Importing…' : '↑ Upload photo'}</button>
