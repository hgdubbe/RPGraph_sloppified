...
The current UI labels the product as `RPgraph Studio v0.5.0 Beta`.
...
The three bundled default workflows are ready-to-use roleplay graphs rather than minimal three-node examples. All three combine `User Input`, `RP Output`, `Chat History`, `Context Compression`, `Event Manager`, an empty `RP Storybook V3` slot, text combiners, a workflow-variable input, and Wire Links. The classic and planning families route through an `LLM Prompt Switch`, which sends Normal RP, Messenger Apps, and Social Media runs into the matching `RP Output` inputs and also provides an Autoplay output connected to the dedicated RP Output Autoplay input; the decision family replaces the Prompt Switch with a Decision Router node driving decision-v1 instead. The classic `workflow.default_vNN.json` family keeps the single-pass prompts, `workflow.default_planning_vNN.json` uses multistep planning for Normal RP and Messenger prompts, and `workflow.default_decision_vNN.json` is the planning graph with the Decision Router in place of the Prompt Switch. Shared graph and format changes are maintained across all three families. Bundled workflows and standalone Storybooks live under `resources/default-content`.
...
Bundled workflow names live in `resources/default-content/` and are versioned independently across three families: the classic `workflow.default_vNN.json`, multistep `workflow.default_planning_vNN.json`, and `workflow.default_decision_vNN.json` (planning plus a Decision Router node, for decision-v1). On startup the Electron layer imports every new bundled workflow and Storybook filename into RPGraph Studio Files. It never overwrites an existing local file, so changed bundled content must use a new versioned filename to appear as an update. On a fresh installation the decision family is selected as the primary default; an existing installation keeps its last active workflow. Loading a workflow whose RP Storybook node is empty opens the local Storybook picker, which can also be dismissed to continue without a Storybook. Files offers `Restore Default Files` whenever either no workflow or no Storybook remains and restores only the missing category.
