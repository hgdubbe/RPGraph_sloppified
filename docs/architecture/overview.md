...
The current UI labels the product as `RPgraph Studio v0.5.5 Beta`.
...
Post publication commands (`Fotogram_text_post`, `Fotogram_image_post`, `OnlyFriends_text_post`, `OnlyFriends_image_post`) run after the visible reply and emit `fotogramPost` or `onlyFriendsPost` objects with `from`, `text`, and an explicit `textOnly` flag. Image posts also require an exact `imageId` from the resolved author’s gallery; text posts reject image IDs. The shared embedded-output parser strips valid and malformed publication payloads from visible prose. `resolveSocialPostCommand` binds an existing enabled account and gallery image, assigns a collision-free per-app post ID, and returns the same `SocialPostRecord` used by the phone composer. `useGraphRun` appends these records through the turn timeline, preserving feed display, history, NPC capture, undo, and reload behavior. A publication may declare a unique `postRef` (letters, digits, underscores, hyphens). Comments and Fotogram/OnlyFriends direct messages in the same output prefix the exact `postRef` value with `new:` instead of guessing the assigned ID. Run-local bindings are scoped by app and populated only after successful publication; duplicate references and failed publications cannot bind a comment. Comments and direct-message origins persist only the resolved real post ID, and ordinary IDs still resolve against history. Both use `resolveSocialPostReference`; an unresolved direct-message reference retains the existing warning-and-deliver-without-context behavior. Commands do not trigger a separate reaction-generation run. Both bundled workflow families place publication hints immediately before social comment hints.
...
New Fotogram and OnlyFriends posts receive an effective-registry account block in User Input context. Eligible NPCs require an enabled user account plus an app-assigned agency tag with public-reaction applicability; creator, disabled, wrong-app, DM-only, unclassified and author accounts are omitted. Each NPC line includes only its exact name, handle and applicable tags. Private runtime instructions tell the model to use those tags for participation and comment style without printing tag labels. Storybook user accounts remain eligible without invented tags; Storybook creator accounts are excluded. Comment-thread and DM routes do not yet use agency behavior. See [NPC Agency Tags](npc-agency-tags.md#candidate-selection-and-prompt-context).
